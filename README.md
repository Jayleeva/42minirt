...
Mais nous verrons cela plus tard, restons meta. Nous envoyons donc des rayons en direction de chaque pixel. Notre programme doit maintenant vérifier si, sur son chemin vers l'infini, le rayon traité rencontre une ou plusieurs formes. Pour ce faire, on lance pour chaque pixel des fonctions "is_this_shape_hit_by_this_ray()" autant de fois que la map annonce de formes, en leur passant le rayon en question (son origine et sa direction), ainsi que la forme testée (sa position et ses dimensions notamment). Nous verrons plus loin le détail des calculs propres à chaque forme. Si ces fonctions nous informent qu'il y a bien intersection avec une ou plusieurs formes, il faut savoir lesquelles, laquelle est la plus proche, et surtout quelle est sa couleur, afin de pouvoir calculer la couleur du pixel concerné.
